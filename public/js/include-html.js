...
$(function ($) {
  ...
  $(".nav-link").each(function () {
    if (this.href === url) {
      $(this).closest(".nav-link").removeClass("text-muted");
...
(function () {
  ...
  window.includeHTML = function (src, destination, callback) {
    ...
    if (src === "places") return includeHTMLplaces;
      ...
      destination = document.body.querySelectorAll(
        'include[src="' +
          destination +
          '"],div[data-src="' +
          destination +
      ...
      if (destination.hasAttribute("data-src"))
        destination.setAttribute("data-src", src);
      if (destination.hasAttribute("data-include"))
        destination.setAttribute("data-include", src);
        ...
        if (status === "error") {
          ...
          iframe.onload = function () {
            if (!response && document.location.protocol.indexOf("http") == -1) {
              let msg;
              if (navigator.userAgent.match(/Chrome/i) !== -1) {
                msg =
                  "For Chrome: add parameter in shortcut link:" +
                  "\n" +
                  '"path_chrome_dirchrome.exe" --allow-file-access-from-files';
              }
              alert(
                navigator.userAgent +
                  "\nResponse: " +
  ...
  function includeHTMLAuto(node) {
    ...
    for (const element of includes) {
      if (element.getAttribute("src"))
        includeHTML(element.getAttribute("src"), element);
      else if (element.getAttribute("data-src"))
        includeHTML(element.getAttribute("data-src"), element);
      else if (element.getAttribute("data-include"))
        includeHTML(element.getAttribute("data-include"), element);
    }
  }
